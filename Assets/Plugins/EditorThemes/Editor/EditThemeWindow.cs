...
public class EditThemeWindow : EditorWindow
{
        ...
        private void OnGUI()
        {
            ...
            if (customView == CustomView.Advanced)
            {
                ...
                foreach (CustomTheme.UIItem I in ct.Items)
                {
            ...
            if (GUILayout.Button("Save", GUILayout.Width(200)))
            {
                ...
                if (ct.Name != Name)
                {
                    ...
                    string Path = Application.dataPath + "/EditorThemes/Editor/StyleSheets/Extensions/CustomThemes/" + ct.Name + ".json";
                    if (File.Exists(Path))
                    {
                        File.Delete(Path);
                    }
                ...
                ThemeSettings.i.SaveJsonFileForCustom(ct);
        ...
        List<Color> CreateAverageCoolors()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                List<string> ColorObjects = ThemeSettings.i.GetColorListByInt(i);
        ...
        void EditColor(int i, Color nc)
        {
            ...
            List<string> edit = ThemeSettings.i.GetColorListByInt(i);
